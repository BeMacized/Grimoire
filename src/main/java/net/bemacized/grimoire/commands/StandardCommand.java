...
import net.bemacized.grimoire.model.controllers.StandardRotation;
...
public class StandardCommand extends BaseCommand {
	...
	public void exec(String[] args, MessageReceivedEvent e) {
		...
		Grimoire.getInstance().getStandardRotation().getSets()
				.parallelStream()
				.collect(Collectors.groupingBy(StandardRotation.StandardSet::getBlock))
				.values()
				.parallelStream()
				.sorted(Comparator.comparing(b -> b.get(0).getEnterDate()))
				.forEachOrdered(b -> {
					final StringBuilder sb = new StringBuilder();
					StandardRotation.StandardSet refSet = b.parallelStream().filter(s -> s.getEnterDate().isAfterNow()).findFirst().orElse(b.get(0));
					// Add release label
					if (refSet.getEnterDate().isAfterNow()) {
