...
public class CompRulesCommand extends BaseCommand {
	...
	public void exec(String[] args, MessageReceivedEvent e) {
		...
		// Verify section
		final ComprehensiveRule section = Grimoire.getInstance().getComprehensiveRules().getRules().parallelStream()
		...
		if (mode > 0) {
			subsection = Grimoire.getInstance().getComprehensiveRules().getRules().parallelStream()
		...
		if (mode > 1) {
			int finalMode = mode;
			paragraph = Grimoire.getInstance().getComprehensiveRules().getRules().parallelStream()
		...
		if (subparagraph != null) {
			...
			long peers = Grimoire.getInstance().getComprehensiveRules().getRules().parallelStream()
		...
		// Paragraph output
			...
			title = String.format("%s %s", section.getParagraphId(), section.getText());
			subtitle = "**" + subsection.getParagraphId() + " " + subsection.getText() + "**";
			List<ComprehensiveRule> rules = Grimoire.getInstance().getComprehensiveRules().getRules().parallelStream()
					.filter(r -> r.getParagraphId().matches(Pattern.quote(paragraph.getParagraphId().substring(0, paragraph.getParagraphId().length() - 1)) + "[a-z.]"))
		...
		// Subsection output
			...
			Supplier<Stream<ComprehensiveRule>> paragraphStream = () -> Grimoire.getInstance().getComprehensiveRules().getRules().parallelStream()
			...
			if (paragraphStream.get().mapToInt(p -> p.getText().split("\\s+").length >= 7 ? 2 : 1).sum() <= 40) {
				paragraphStream.get().forEachOrdered(p -> {
					...
					if (textlength < 7) {
						...
						description.append("\n:small_orange_diamond: **" + p.getParagraphId() + "** " + underlineKeywordsAndParseEmoji(p.getText(), e.getGuild()));
						if (Grimoire.getInstance().getComprehensiveRules().getRules().parallelStream().filter(c -> c.getParagraphId().matches(Pattern.quote(p.getParagraphId().substring(0, p.getParagraphId().length() - 1)) + "[a-z]")).count() > 0)
