...
public abstract class CardBaseCommand extends BaseCommand {
    ...
    public void exec(String[] args, String rawArgs, MessageReceivedEvent e, GuildPreferences guildPreferences) {
        ...
        if (card != null && !card.getLanguage().equalsIgnoreCase("English")) results.add(card);
        ...
        execForCards(results, loadMsg, e, guildPreferences);
    }

    protected abstract String getInitialLoadLine();

    protected abstract void execForCards(List<MtgCard> card, LoadMessage loadMsg, MessageReceivedEvent e, GuildPreferences guildPreferences);
}
