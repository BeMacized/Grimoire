...
import type { ChatTools } from '../src/ChatProcessor';
...
describe('ChatProcessor', () => {
  describe('Card reference extraction', () => {
    it('properly extracts an inline card name.', (done) => {
      ...
      const chatTools: ChatTools = { sendFile: async () => ({}), sendMessage: async () => ({}) };
      const chatProcessor: ChatProcessor = new ChatProcessor(chatTools, {}, defaultConfig, ({ setLastMentioned: () => {} }: any), ({ processMessage: () => {} }: any));
  ...
  describe('Logging', () => {
    it('sets the last card as the last mentioned card for the channel', (done) => {
      ...
      // Instantiate chatProcessor
      const appState: any = { setLastMentioned: (channelId: string, cardId: string, setCode: string) => {
        assert.equal(channelId, 'channel id', 'Given channel id is not the one we expected');
        assert.equal(cardId, 'example id', 'Given card id is not the one we expected');
        assert.equal(setCode, 'set id', 'Given set id is not the one we expected');
        done();
      } };
      const chatProcessor: ChatProcessor = new ChatProcessor(chatTools, mtg, defaultConfig, appState, ({ processMessage: () => {} }: any));
  ...
  describe('Library interaction', () => {
    it('attempts uploading card art for inline cards', (done) => {
      ...
      // Instantiate chatProcessor
      const chatProcessor: ChatProcessor = new ChatProcessor(chatTools, mtg, defaultConfig, ({ setLastMentioned: () => {} }: any), ({ processMessage: () => {} }: any));
